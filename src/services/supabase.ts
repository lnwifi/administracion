...
export const userMembershipsService = {
  async getAll() {
    ...
    // Fetch user names separately (no direct FK between user_memberships and profiles)
...
export const appConfigService = {
  ...
  async updateConfig(key: string, config: Record<string, any>) {
...
export const couponsService = {
  ...
  async validateCode(code: string, placeId: string) {
  ...
  async getRedemptionsByPlace(placeId: string) {
    const { data, error } = await supabaseAdmin
      .from('user_coupons')
      .select('*, coupons!user_coupons_coupon_id_fkey(title)')
      .eq('place_id', placeId)
...
export const authService = {
  async createOwnerUser(email: string, password: string, fullName: string) {
    ...
    // Si el email ya existe, buscarlo y actualizar su contraseña
  ...
  async updateOwnerPassword(userId: string, newPassword: string) {
...
export const dashboardService = {
  async getStats() {
    try {
      ...
      const recentOrders = await safeSelect('pedidos', 'id, total, estado, fecha_pedido', 5, 'fecha_pedido');
...
export const analyticsService = {
      ...
      if (recentUsers) {
        recentUsers.forEach(user => {
          activities.push({
            ...
            description: user.full_name || 'Usuario',
  ...
  async getPeriodStats(startDate: Date, endDate?: Date) {
    ...
    const countQuery = endDateStr
      ? (table: string, dateCol = 'created_at') => supabaseAdmin.from(table).select('id', { count: 'exact', head: true }).gte(dateCol, startDateStr).lt(dateCol, endDateStr)
      : (table: string, dateCol = 'created_at') => supabaseAdmin.from(table).select('id', { count: 'exact', head: true }).gte(dateCol, startDateStr);
    ...
    try {
      const [
        ...
        countQuery('profiles'),
        countQuery('pets'),
        countQuery('pedidos', 'fecha_pedido')
      ]);
...
export const notificationsService = {
  async getNotifications() {
    try {
      // Solo obtener notificaciones del admin (user_id especial y from_admin: true)
      const { data, error } = await supabaseAdmin
        .from('notifications')
        .select('*')
      ...
      // Transform data to match expected interface - solo notificaciones del admin
      return (data || [])
        .filter(notification => notification.metadata?.from_admin === true)
        .map(notification => ({
          ...notification,
  ...
  async createNotification(notification: { title: string; content: string; type: string; channel: string; target_audience: string; scheduled_for?: string | null; specific_users?: string[] }) {
    try {
      ...
      const userId = '00000000-0000-0000-0000-000000000000'; // ID especial para notificaciones del admin
  ...
  async updateNotification(id: string, notification: Partial<{ title: string; content: string; type: string; channel: string; target_audience: string; scheduled_for?: string | null; updated_at: string }>) {
    try {
      // Get raw notification data to preserve original metadata
      const { data: currentRaw } = await supabaseAdmin
        .from('notifications')
        .select('*')
        .eq('id', id)
        .single();

      if (!currentRaw) throw new Error('Notificación no encontrada');
    ...
    try {
      ...
      const updateData: any = {
        metadata: {
          ...
          sent_at: stats.sent_at || null,
          recipient_count: stats.recipient_count || 0,
    ...
    try {
      ...
      switch (targetAudience) {
        ...
        case 'premium_users':
          ...
          const premiumUserIds = (activeMemberships || []).map((membership: any) => membership.user_id);
  ...
  async estimateRecipientCount(targetAudience: string, specificUsers?: string[]): Promise<number> {
    try {
      const recipients = await this.getRecipients(targetAudience, specificUsers);
